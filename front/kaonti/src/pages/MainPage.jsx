...
const [compte, setCompte] = React.useState(decoded.UserInfo.compte || 'Espace client non paramétré');
const [userName, setUserName] = React.useState(decoded.UserInfo.username || 'Invité');
const [parentHorinzotalWidth, setParentHorinzotalWidth] = useState("95vw")
//const {idDossier, setIdDossier} = useFileInfos();
...
const handleDrawerOpen = () => {
  setOpen(true);
  setDwidth(200);
  setParentHorinzotalWidth("86vw");
};

const handleDrawerClose = () => {
  setOpen(false);
  setDwidth(75);
  setParentHorinzotalWidth("95vw");
};
...
const ProfileImage = ({ name }) => {
  ...
  return (
    <span className="user-profile-image" style={{height:"50px", width:"50px", fontSize:"16px", backgroundColor:"transparent"}}>
      {firstNameInitial}
      {lastNameInitial}
    </span>
  );
...
function showSubMenu(name, subMenu, path, urlDynamic){
  if(subMenu == null){
    ...
    if(urlDynamic){
      ...
      const pathWithId = `${path}/${idDossier}`;
      navigate(pathWithId);
    }else{
      navigate(path);
    }
...
//------------------------------------------------------------------------------------------------------------
//Calcul de la taille window après resizing
...
function handleSubmitMdpChangeFormik(formValues, onSubmittingProps){
  ...
  return (
    <Box sx={{ display: 'flex', position: "relative", height:'95%', width:'10%'}}>
      <CssBaseline />
      <AppBar position="fixed" open={open} style={{height:"30px"}}>
        <Toolbar style={{backgroundColor: "#010122", alignContent:'flex-start', alignItems:"center"}} variant="dense">
          ...
          <Stack direction={"row"} justifyContent={"flex-start"} alignContent={"center"} alignItems={"center"} marginLeft={"10px"}>
          ...
          <Stack direction={"row"} alignItems={"right"} justifyContent={"flex-end"} width={"100%"}>
            ...
            <div >
              ...
              >
                <div className="main" style={{
                  fontWeight:"lighter", height:"35px", 
                  width:"35px", backgroundColor:"#427AA1", 
                  borderRadius:"50%",
      ...
      <Drawer variant="permanent" open={open} style={{marginRight:"-10px"}}
        PaperProps={{
          sx: {
        ...
        <List >
            {MenuSide.map(item =>(
              ...
              >
                <ListItemButton style={{marginLeft:"-12px", marginBottom:'-15px'}}>
            ...
            <Tooltip title="Se déconnecter de l'application">
              <IconButton
                style={{
                  ...
                  borderColor: "transparent", backgroundColor:'red',
                  textTransform: 'none', outline: 'none'
                }}
                aria-label="close"
                onClick={Disconnect}
        ...
        >
          ...
          <DialogContent dividers>
            ...
            >
              ...
              value={mdpChangeFormik.values.oldPassword}
              onChange={mdpChangeFormik.handleChange}
              onBlur={mdpChangeFormik.handleBlur}
              />*/}
              <TextField style={{width: "300px"}} id="standard-basic1" 
              label="Mot de passe actuel" variant="standard" name='oldPassword' 
      ...
      {/* SOUS MENU */}
      <div style={{position:"initial", width:parentHorinzotalWidth, minWidth:parentHorinzotalWidth, height:"90vh"
                  , marginTop:"10px", marginLeft:"10px", marginBottom:"0px", zIndex:"1"}}>
        {subMenuState.administration ? <Administration humburgerMenuState={open} onWindowState={handleCloseSubMenu} pathToNavigate={subMenuPathNavigation}/> : null}
        {subMenuState.revision ? <Revisions humburgerMenuState={open} onWindowState={handleCloseSubMenu} pathToNavigate={subMenuPathNavigation}/> : null}
        {subMenuState.declaration ? <Declaration humburgerMenuState={open} onWindowState={handleCloseSubMenu} pathToNavigate={subMenuPathNavigation}/> : null}
        {subMenuState.parametrages ? <Parametrages humburgerMenuState={open} onWindowState={handleCloseSubMenu} pathToNavigate={subMenuPathNavigation}/> : null}
        <Outlet />
      </div>
    </Box>
  );
