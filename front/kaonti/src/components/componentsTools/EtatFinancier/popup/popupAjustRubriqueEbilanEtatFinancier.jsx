...
const PopupAjustRubriqueEbilanEtatFinancier = ({ actionState, row, column, setIsRefreshed, value }) => {
    ...
    const getInfosAjust = (compteId, dossierId, exerciceId, etatId, rubriqueId, nature) => {
        axios.get(`/administration/etatFinancier/getAjustementExterne`, {
            params: {
    ...
    const deleteRow = (value) => {
        if (value === true) {
            if (selectedRowId?.length === 1) {
                ...
                axios.delete(`/administration/etatFinancier/deleteAjustementExterne/${Number(idToDelete)}`).then((response) => {
                    const resData = response.data;
...
export default PopupAjustRubriqueEbilanEtatFinancier;
