...
const VirtualTableSADEbilan = ({ refreshTable, columns, rows, noCollapsible, state }) => {
  ...
  const totalColumnDetail = (rows, columnId) => {
    let data = [];
    if (columnId === 'n') {
      data = rows.filter(item => item.nature === 'N');
    } else if (columnId === 'n1') {
                    ...
                    {columns.map((column, idx) => {
                      ...
                      if (column.id === targetColumnId) {
                        ...
                        return (
                          ...
                          >
                            {column.isNumber
                              ? (row.ajustsSAD && row.ajustsSAD.length > 0 && totalColumnDetail(row.ajustsSAD, column.id) !== 0)
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <FormControl size="small" fullWidth style={{ marginBottom: '0px', width: '250px' }}>
                              ...
                              >
                                ...
                                <MenuItem key="N" value="N">Colonne N</MenuItem>
                          ...
                          {(row.ajustsSAD.filter(colonne === 'TOUS' ? item => item.nature !== colonne : item => item.nature === colonne) && row.ajustsSAD.filter(colonne === 'TOUS' ? item => item.nature !== colonne : item => item.nature === colonne).length > 0) ? (
                            <Table size="small" aria-label="details">
                              ...
                              <TableBody>
                                {row.ajustsSAD.filter(colonne === 'TOUS' ? item => item.nature !== colonne : item => item.nature === colonne).map((ajust, index) => (
                                  <TableRow
                                    key={index}
                                    style={{
                              ...
                              >
                                <TableRow style={{ border: 'none' }}>
                                  ...
                                  >
                                    {
                                      totalColumnAjust(row.ajustsSAD.filter(colonne === 'TOUS' ? item => item.nature !== colonne : item => item.nature === colonne), "montant").toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
                                    }
                                  </TableCell>
                                </TableRow>
                              </TableFooter>
...
export default VirtualTableSADEbilan;