          ...
          <TableBody>
            {rows.map((group) => (
              <React.Fragment key={group.rubriques_poste} >
                ...
                <TableRow>
                  {columns.map((column) => {
                    // Colonne spéciale pour le groupe avec icône
                    if (column.id === 'rubriques_poste') {
                      const label =
                        group.rubriques_poste === "GOODWILL" ? "GoodWill" :
                          group.rubriques_poste === "IMMO_CORP" ? "Immobilisations corporelles" :
                            group.rubriques_poste === "IMMO_INCORP" ? "Immobilisations incorporelles" :
                              group.rubriques_poste === "IMMO_ENCOURS" ? "Immobilisations en cours" :
                                "Immobilisaions financières";
                    ...
                    const numericColumns = [
                      'valeur_acquisition', 'augmentation', 'diminution',
                      'amort_anterieur', 'dotation_exercice', 'amort_cumule', 'valeur_nette'
                    ];
                  ...
                  <TableCell colSpan={columns.length + 1} sx={{ p: 0 }}>
                    ...
                    >
                      <Table size="small" sx={{ tableLayout: "fixed", width: "100%" }}>
                        ...
                        <TableBody>
                          {group.items.sort((a, b) => { return b.augmentation - a.augmentation; }).map((item) => (
...
export default VirtualTableModifiableGroupableEbilanDA;