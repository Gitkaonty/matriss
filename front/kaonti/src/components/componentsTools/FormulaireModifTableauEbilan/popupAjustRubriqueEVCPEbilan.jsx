...
const popupAjustRubriqueEVCPEbilan = ({ actionState, row, column, value, canModify, canAdd, canDelete, canView }) => {
    ...
    useEffect(() => {
        if (canView) {
            setListAjust(row.ajustsEVCP.filter(item => item.nature === nature));
        }
    }, [row]);
    ...
    const handleSaveClick = (id) => () => {
        ...
        axioPrivate.post(`/declaration/ebilan/addModifyAjust`, newFormDataFinal).then((response) => {
            const resData = response.data;
            if (resData.state) {
    ...
    const deleteRow = (value) => {
        if (value === true) {
            if (selectedRowId.length === 1) {
                ...
                axioPrivate.post(`/declaration/ebilan/deleteAjust`, { idCompte, idDossier, idExercice, idEtat, idRubrique, nature, idToDelete }).then((response) => {
                    const resData = response.data;
...
export default popupAjustRubriqueEVCPEbilan;
