...
const popupAjustRubriqueSDREbilan = ({ actionState, row, column, value }) => {
    ...
    const data = { ...row };
    const rubriqueCaption = data['libelle'].replace('Constitution Solde', 'Solde');
    ...
    useEffect(() => {
        ...
        if (column === 'exercice') {
            ...
            setHeaderLabel('N-6');
        } else if (column === 'solde_imputable') {
            setHeaderLabel('Solde imputable');
        } else if (column === 'solde_non_imputable') {
            setHeaderLabel('solde non imputable');
        }
    }, [listAjust]);
    ...
    useEffect(() => {
        setListAjust(row.ajustsSDR.filter(item => item.nature === nature));
    }, [row]);
            ...
            >
                <DialogTitle sx={{ ml: 1, p: 2 }} id="customized-dialog-title" style={{ fontWeight: 'normal', width: '850px', height: '50px', backgroundColor: 'transparent' }}>
                    <Typography variant={'h8'} style={{ fontZise: 10 }}>
                        Ajustement - Constitution {rubriqueCaption} / Imputation {headerLabel}
                    </Typography>
                </DialogTitle>
...
export default popupAjustRubriqueSDREbilan;
