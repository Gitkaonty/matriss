...
export default function ConsultationComponent() {
    ...
    const [valSelect, setValSelect] = useState('tout');
    ...
    const handleSearch = () => {
        if (!valSelect || valSelect === 'tout') {
            console.log('Filtrage désactivé, valSelect = "tout"');
            setFilteredList([]);
            return;
        }

        const compteSelect = listePlanComptable.find(item => item.id === valSelect);
        if (!compteSelect) {
            setFilteredList([]);
            return;
        }
    ...
    const handlePrevious = () => {
        const currentIndex = listePlanComptable.findIndex(item => item.id === valSelect);
        if (currentIndex > 0) {
            setValSelect(listePlanComptable[currentIndex - 1].id);
        } else if (currentIndex === 0) {
            setValSelect("tout"); // Revenir à "tout"
        }
    };
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (filtrageCompte === "0") {
                // Tous les comptes : on restaure à partir de l'initiale
                    ...
                    <Stack width={"100%"} height={"100%"} spacing={6} alignItems={"flex-start"} alignContent={"flex-start"} justifyContent={"stretch"}>
                        ...
                        >
                            ...
                            >
                                <Stack direction={'row'} alignContent={'center'}>
                                    <Stack
                                        ...
                                        >
                                            <MenuItem value="tout">
                                                Sélectionner une compte
                                            </MenuItem>
                                            {
                                                listePlanComptable.map((val) => (
                                                    <MenuItem key={val.id} value={val.id}>
                                                        {`${val.compte || ''} - ${val.libelle || ''}`}
                                                    </MenuItem>
                                    ...
                                    <Stack direction={'row'} spacing={1}>
                                        ...
                                        <Button
                                            disabled={
                                                listePlanComptable.findIndex(item => item.id === valSelect) >= listePlanComptable.length - 1
                                            }
                                            sx={{
                                                minWidth: 0,
                                                padding: 1,
                            ...
                            <RadioGroup
                                ...
                                name="filtrageCompte"
                                onChange={(e) => setFiltrageCompte(e.target.value)}
                        ...
                        <Stack
                            ...
                            >
                                <TbPlugConnected size={20} style={{ marginRight: 4 }} />Lettrer
                            </Button>
                            <Button
                                disabled={selectedRows.length === 0 || solde !== 0}
                                variant="contained"
                            ...
                            >
                                <TbPlugConnectedX size={20} style={{ marginRight: 5 }} />Délettrer
                            </Button>
