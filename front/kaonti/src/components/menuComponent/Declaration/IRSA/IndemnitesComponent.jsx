...
export default function IndemnitesComponent() {
    ...
    const indemniteColumns = [
        ...
        { field: 'nonImposables', headerName: 'Non imposables', width: 180, editable: true, type: 'number' },
    ];

    const fetchIndemnites = () => {
        console.log('Fetching indemnites...');
        axios.get(`/irsa/indemnite`)
            .then(res => {
                console.log('Indemnites response:', res.data);
                const data = res.data.state && Array.isArray(res.data.list) ? res.data.list.map((item, idx) => ({
                    id: item.id || idx + 1,
                    imposables: item.imposables,
                    nonImposables: item.nonImposables
                })) : [];
                console.log('Processed indemnites data:', data);
                setRows(data);
            })
            .catch((error) => {
                console.error('Error fetching indemnites:', error);
                setRows([]);
            });
    };
    ...
    const handleOpenDialogAddNew = () => {
        ...
        const newIndemnite = { id: newId, imposables: '', nonImposables: '' };
        setRows([...rows, newIndemnite]);
        setNewRow(newIndemnite);
        setEditRow(null);
    ...
    const handleSaveClick = (ids) => () => {
        if(editRow){
            axios.put(`/irsa/indemnite/${editRow.id}`, editRow)
                .then(res => {
                    if(res.data && res.data.state){
                        ...
                        setDisableCancelBouton(true);
                        setDisableDeleteBouton(true);
                        toast.success(res.data.msg || 'Indemnité modifiée');
                    }else{
            ...
            axios.post(`/irsa/indemnite`, newRow)
                .then(res => {
                    if(res.data && res.data.state){
                        fetchIndemnites();
                        setEditRow(null);
                        ...
                        setDisableCancelBouton(true);
                        setDisableDeleteBouton(true);
                        toast.success(res.data.msg || 'Indemnité ajoutée');
                    }else{
    ...
    const deleteRow = (value) => {
        if(value === true && selectedRowId.length === 1){
            ...
            axios.delete(`/irsa/indemnite/${idToDelete}`)
                .then(res => {
                    if(res.data && res.data.state){
                        ...
                        setDisableCancelBouton(true);
                        setDisableDeleteBouton(true);
                        toast.success(res.data.msg || 'Indemnité supprimée');
