...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    const rows = [
      ...
      // Groupe 4 - Crédit et régularisations
      ...
      // Groupe 5 - TVA nette et crédit à reporter
    ...
    const existingRecords = await queryInterface.sequelize.query(
      'SELECT id_code FROM formulaire_tva_annexes_matrices WHERE id_code IN (:codes)',
      {
        replacements: { codes: rows.map(r => r.id_code) },
        type: queryInterface.sequelize.QueryTypes.SELECT
      }
    );

    const existingCodes = existingRecords.map(r => r.id_code);
    const newRows = rows.filter(r => !existingCodes.includes(r.id_code));

    if (newRows.length > 0) {
      await queryInterface.bulkInsert('formulaire_tva_annexes_matrices', newRows);
      console.log(`Inserted ${newRows.length} new records into formulaire_tva_annexes_matrices`);
    } else {
      console.log('All records already exist in formulaire_tva_annexes_matrices');
    }
  },
  ...
  async down(queryInterface, Sequelize) {
    ...
    await queryInterface.bulkDelete('formulaire_tva_annexes_matrices', {
      id_code: { 
        [Op.in]: [
          100,102,103,105,106,107,108,115,125,130,140,150,155,160,161,165,170,180,
    ...
    console.log('Removed formulaire_tva_annexes_matrices seed data');
  }
};
