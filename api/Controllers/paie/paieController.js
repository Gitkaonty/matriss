...
exports.exportPaieTemplate = (req, res) => {
  ...
  const columns = [
    'personnel_id', 'mois', 'annee', 'salaireBase', 'prime', 'heuresSup', 'indemnites', 'remunerationFerieDimanche',
    'assurance', 'carburant', 'entretienReparation', 'loyerMensuel', 'depenseTelephone', 'autresAvantagesNature',
    'avanceQuinzaineAutres', 'avancesSpeciales', 'allocationFamiliale', 'deductionEnfants', 'nombre_enfants_charge'
  ];
      ...
      include: [
        {
          model: Personnel,
          as: 'personnel',
          attributes: ['id', 'nom', 'prenom', 'id_fonction', 'id_classe', 'date_entree', 'date_sortie'],
          include: [
            {
              model: Fonction,
              as: 'fonction',
...
exports.create = async (req, res) => {
  try {
    ...
    if (!req.body.personnelId) {
      return res.status(400).json({ state: false, msg: 'personnelId est requis' });
    }
